...
-- 导入表
copy "public".PhotoObjAll from '/home/gpadmin/load/comma/PhotoObjAll.csv' with DELIMITER ',' csv header;
-- linux shell tab
/usr/bin/time -v -o runPhoObj.txt psql -d astronomy -c "copy PhotoObjAll from '/home/gpadmin/load/tab/PhotoObjAll10.csv' with DELIMITER '\t';"  
...
explain analyze Select G.objID, G.u, G.g, G.r, G.i, G.z from (SELECT * FROM ( SELECT * FROM PhotoObjAll WHERE mode=1) as p WHERE type = 3) as G, (SELECT * FROM ( SELECT * FROM PhotoObjAll WHERE mode=1) as h) as S where G.parentID > 0 and G.parentID = S.parentID;
-- 两表查询 photoobjall/starlj
