...
-- 导入表
copy "public".Star from '/home/gpadmin/load/comma/Star.csv' with DELIMITER ',' csv header;
-- linux shell tab
/usr/bin/time -v -o runStr.txt psql -d astronomy -c "copy StarLJ from '/home/gpadmin/load/tab/10G/StarLJ10.csv' with DELIMITER '\t';"  

-- 单表查询
explain analyze SELECT run, camcol, rerun, field, objID, u, g, r, i, z, ra, dec FROM StarLJ WHERE ( u - g > 2.0 or u> 22.3 ) and ( i < 19 ) and ( i > 0 ) and ( g - r > 1.0 ) and ( r - i < (0.08 + 0.42 * (g - r - 0.96)) or g - r > 2.26 ) and ( i - z < 0.25 );